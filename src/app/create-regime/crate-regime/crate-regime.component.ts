...
export class CrateRegimeComponent {
  ...
  mostrarObjetivosSelecionados() {
    ...
    const objetivosSelecionados = this.selectedObjective.filter(
      (objetivo: any) => objetivo.objetivo === selectedObjective
    );
  
    const imcCalculado = this.calculateIMC();
  
    if (objetivosSelecionados.length > 0) {
      const objetivoExcedente = objetivosSelecionados.find((objetivo: any) => {
      ...
      if (objetivoExcedente) {
        ...
        this.snackBar.open('Nenhum objetivo correspondente encontrado para o IMC calculado.', 'Fechar', {
          duration: 5000,
      ...
      this.snackBar.open('Ocorreu um erro ao buscar os objetivos.', 'Fechar', {
        duration: 5000,
        verticalPosition: 'top',
        horizontalPosition: 'center', 
      });
